// Meer: reisinstellingen (regressie) en de nieuwe to-do-lijst.
...
test('taak toevoegen met deadline, toewijzing, categorie en dag', async ({ page }) => {
  ...
  const db = await opMeer(page, { days: dagen });
  ...
  await expect(page.getByText('Ilse')).toBeVisible();
...
test('taak afvinken via het selectievakje, zonder de dialoog te openen', async ({ page }) => {
  const db = await opMeer(page, { tasks: [{ id: 't1', trip_id: REIS.id, titel: 'Paspoorten controleren', klaar: false, created_at: 't1' }] });
...
test('taak bewerken en verwijderen', async ({ page }) => {
  const db = await opMeer(page, { tasks: [{ id: 't1', trip_id: REIS.id, titel: 'Grensvergunning', klaar: false, created_at: 't1' }] });
